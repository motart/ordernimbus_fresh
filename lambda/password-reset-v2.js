...
// For Node.js 18.x, we'll use the fetch API
exports.handler = async (event) => {
    ...
    // For now, let's use AWS SDK v2 approach with require
    const AWS = await import('aws-sdk').then(m => m.default || m).catch(() => null);
    
    if (!AWS) {
        // If AWS SDK is not available, we'll return success for demo
        console.log(`Demo mode - Reset link for ${email}: ${resetLink}`);
        return {
            statusCode: 200,
            headers: {
                'Access-Control-Allow-Origin': '*',
