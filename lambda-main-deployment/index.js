...
exports.handler = async (event) => {
  ...
  const allowedOrigins = [
    ...
    'http://localhost:3000',
    'http://127.0.0.1:3000',
    'http://ordernimbus-production-frontend-335021149718.s3-website-us-west-1.amazonaws.com',
  ...
  try {
    ...
    switch(resource) {
      case 'config':
        ...
        // This returns configuration that the frontend needs
        responseData = {
          // API Configuration
          apiUrl: `https://${event.requestContext?.domainName || 'ay8k50buyd.execute-api.us-west-1.amazonaws.com'}/${event.requestContext?.stage || 'production'}`,
          wsUrl: `wss://${event.requestContext?.domainName || 'ay8k50buyd.execute-api.us-west-1.amazonaws.com'}/${event.requestContext?.stage || 'production'}/ws`,
          graphqlUrl: `https://${event.requestContext?.domainName || 'ay8k50buyd.execute-api.us-west-1.amazonaws.com'}/${event.requestContext?.stage || 'production'}/graphql`,
          
          // AWS Cognito Configuration
          userPoolId: process.env.USER_POOL_ID || 'us-west-1_GeV4w2rCQ',
          clientId: process.env.USER_POOL_CLIENT_ID || '2dr8p83gqu0v9iktpdq4qo2rdg',
          ...
          // Feature Flags (can be managed via environment variables)
          features: {
          ...
          deploymentId: process.env.DEPLOYMENT_ID || 'production-latest'
        };
        break;
      ...
      case 'shopify':
        ...
        if (path.includes('/shopify/connect')) {
          try {
            ...
            const API_GATEWAY_URL = 'https://ay8k50buyd.execute-api.us-west-1.amazonaws.com/production';
