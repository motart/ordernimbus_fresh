...
# Fix Shopify data synchronization in Lambda
...
echo "Fixing Shopify data synchronization..."
...
RESPONSE=$(curl -s "$API_URL/api/stores" -H "userId: test" | jq -c '. | {count, storesReturned: (.stores | length)}')
echo "Response: $RESPONSE"

echo ""
echo "✅ Shopify data synchronization fix applied!"
echo ""
echo "The Lambda now:"
echo "  • Fetches shop info, products, orders, and customers counts after OAuth"
echo "  • Properly formats store data with type, status, and sync metadata"
echo "  • Handles store deletion (DELETE method)"
echo "  • Returns properly structured store objects for the frontend"
echo ""
echo "To test:"
echo "  1. Clear browser cache at https://app.ordernimbus.com/clear-cache.html"
echo "  2. Connect a new Shopify store"
echo "  3. The store should appear with proper data and counts"