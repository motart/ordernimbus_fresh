...
# OrderNimbus Local Development Startup Script (No Docker for SAM)
# Starts all services needed for local development without Docker mount issues
...
echo -e "${PURPLE}🚀 Starting OrderNimbus Local Development Environment (No Docker SAM)${NC}"
echo "=================================================="
...
# Start Docker services (only for DynamoDB, LocalStack, etc.)
...
# Build SAM application without Docker
echo -e "${BLUE}🏗️  Building SAM application (no container)...${NC}"
sam build

# Start SAM Local API in background without container
echo -e "${BLUE}⚡ Starting SAM Local API (no container)...${NC}"
sam local start-api \
    --env-vars env.json \
    --port 3001 \
    --host 127.0.0.1 &
